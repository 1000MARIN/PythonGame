...
# 화면 타이틀(게임이름) 설정
pygame.display.set_caption("Lee Pang")
...
stage_height = stage_size[1]    # 스테이지의 노ㅠ이 위에 캐릭터를 두기 위해 사용
...
balls.append({
    ...
    "to_y" : -6,    # y축 이동방향
    "init_spd_y" : ball_speed_y[0]  # y 공의 최초 속도
})
...
while running:
    ...
    for ball_idx, ball_val in enumerate(balls):
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                if ball_img_idx < 3:
                    ...
                    balls.append({
                        ...
                        "to_x" : -3,     # x축 이동방향, -3 이면 왼쪽으로 3 이면 오른쪽으로
                        "to_y" : -6,    # y축 이동방향
                        "init_spd_y" : ball_speed_y[ball_img_idx + 1]  # y 공의 최초 속도
                    })
        ...
        break           # 안쪽 for 문에서 break 를 만나면 여기로 진입 가능. 2중 for 문을 한번에
